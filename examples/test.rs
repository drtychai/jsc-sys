...
fn rust_function_defn(name: &String) -> JSClassDefinition {
    JSClassDefinition {
        ...
        callAsFunction: None, //Some(callback_trampoline as u64),
...
//        let size: u64 = unsafe { JSStringGetMaximumUTF8CStringSize(string.0) };
//        let mut buffer: usize = vec![0u8; size.try_into().unwrap()];
//        let written = unsafe { JSStringGetUTF8CString(string.0, buffer.as_mut_ptr() as _, size as u64) };
